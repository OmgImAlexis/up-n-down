...
const get = async (req, res) => {
    ...
    const { rows: [row] } = await db.getPostWithPublic2(postPublicId, getCurrentTimezone(req), finalUserId, filterUserId);
    ...
    const { rows: comments } = await db.getPostComments(row.post_id, getCurrentTimezone(req), finalUserId, isDiscoverModeEnabled, filterUserId, page);

    // Set the title to depending on the user's permissions
    const title = row.is_visible ? row.title : 'Post #' + row.public_id;
...
const post = async (req, res) => {
    ...
    // Render an error page if they're not allowed
    const isAllowed = await db.isAllowedToViewPost(post.private_group_ids, req.session.user.user_id);
    if(!isAllowed) {
        return res.render('message', {

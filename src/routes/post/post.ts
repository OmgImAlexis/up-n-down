...
export const postPost = async (request: Request, response: Response) => {
	...
	try {
		...
		const isAllowed = await isUserAllowedToViewPost(post.private_group_ids, request.session.user.user_id);
		if (!isAllowed) {
		...
		const comment = await createPostComment(post.post_id, request.session.user.user_id, content);
